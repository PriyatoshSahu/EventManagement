...
//		Long eventStartTime = Long.valueOf(request.getStartDate());
	    ...
	    try {
	        eventStartDate = LocalDate.parse(request.getStartDate());
	    } catch (Exception e) {
	        response.setResponseMessage("Invalid date format for startDate");
	        response.setSuccess(false);
	        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
	    }

	    LocalDateTime now = LocalDateTime.now();
	    if (eventStartDate.isBefore(now.toLocalDate())) {
...
//		}
		...
		event.setTotalTicket(request.getNoOfTickets());
	...
	public ResponseEntity<CommonApiResponse> updateEvent(AddEventRequestDto request) {
		...
		event.setStartDate(dbEvent.getStartDate());
		event.setTicketAvailable(request.getAvailableTickets());
	...
	public ResponseEntity<EventResponseDto> searchActiveEventsByName(String eventName) {
		...
		List<Event> events = this.eventService.findByStatusAndNameContainingIgnoreCaseAndStartDateGreaterThan(
